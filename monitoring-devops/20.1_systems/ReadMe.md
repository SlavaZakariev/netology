...
8. Перейдите в веб-интерфейс Chronograf (http://localhost:8888) и откройте вкладку Data explorer.
    ...
    - В `measurments` выберите **cpu>host>telegraf-getting-started**, а в `fields` выберите **usage_system**. Внизу будет график **cpu**.
    - Вверху вы можете увидеть запрос, аналогичный SQL-синтаксису. Попробуйте изменить группировку и интервал наблюдений.
...
Система сбора и визуализации логов как кластер ELK, сервис позволяет собирать и визуализировать логи, или же связку Grafana Loki для сбора логов + Grafana для визуализации, в дополнение можно рассмотреть связку Prometheus для сбора логов + Grafana для визуализации. Программный продукт Sentry поможет перехватывать ошибки приложений. 
...
- Настройка точек приёма осуществляется на агентах, что позволяет настроить вывод метрик в несколько систем мониторинга.
...
- **Prometheus** - использует **pull** модель, получает данные с экспортеров, которые установлены на хосты под контроль системы.
